...
import { SampleInfo, Sample, SampleOptions } from '../types/index.js';
 ...
 */
export function extractSamples(xmlRoot: any, options: SampleOptions = {}): SampleInfo {
 ...
 */
function parseSample(fileRef: any, customSplicePaths?: string[]): Sample | null {
 ...
 */
function isAudioSample(path: string, originalFileSize?: string | number): boolean {
  ...
  const isBuiltInDevice =
    !hasAudioExtension && (path === 'Tuner' || path === 'External Instrument' || isDevicePath);
  ...
  return (
    (hasValidExtension || hasNonZeroSize) && !isDevicePath && !isBuiltInDevice && !isDeviceFile
  );
}
 ...
 */
function isSpliceSample(path: string, customPaths?: string[]): boolean {
 ...
 */
function extractPackName(path: string): string | undefined {
 ...
 */
function deduplicateSamples(samples: Sample[]): Sample[] {
