 ...
 */
export function extractScale(xmlRoot) {
    ...
    const scales = scaleElements.map(parseScaleElement).filter((s) => s !== null);
 ...
 */
function parseScaleElement(element) {
 ...
 */
function getUniqueScales(scales) {
    const seen = new Set();
    const unique = [];
 ...
 */
function getScaleDistribution(scales) {
    const distribution = {};
    for (const scale of scales) {
