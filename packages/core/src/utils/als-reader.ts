 ...
 */
export async function readAlsFile(filePath: string): Promise<string> {
 ...
 */
export function readAlsBuffer(buffer: Buffer): string {
  try {
    const decompressed = gunzipSync(buffer);
    return decompressed.toString('utf-8');
  } catch (error) {
    throw new Error(
      `Failed to decompress .als buffer. It may be corrupted or not a valid Ableton Live Set file. ${error}`
    );
  }
}
